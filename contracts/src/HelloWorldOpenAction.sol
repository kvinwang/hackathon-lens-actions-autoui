...
contract HelloWorldOpenAction is
    ...
    LensModuleMetadata
{
    mapping(uint256 profileId => mapping(uint256 pubId => string initMessage))
        internal _initMessages;
    ...
    function initializePublicationAction(
        ...
        (string memory id, string memory domain) = abi.decode(data, (string, string));

        _initMessages[profileId][pubId] = string.concat(id, "@", domain);
    ...
    function processPublicationAction(
        ...
        string memory initMessage = _initMessages[
            params.publicationActedProfileId
        ][params.publicationActedId];
        string memory actionMessage = abi.decode(
            params.actionModuleData,
            (string)
        );

        bytes memory combinedMessage = abi.encodePacked(
            initMessage,
            " ",
            actionMessage
        );
